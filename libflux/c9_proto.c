...
c9parsedir(C9ctx *c, C9stat *stat, uint8_t **t, uint32_t *size)
{
	...
	*t += 6; /* skip type(2) and dev(4) */
	...
	if((cnt = r16(t)) > sz-2)
		goto error;
	stat->name = (0 == cnt) ? NULL : (char*)*t; b = *t = *t+cnt; sz -= 2+cnt;
	if(sz < 2 || (cnt = r16(t)) > sz-2)
		goto error;
	stat->uid = (0 == cnt) ? NULL : (char*)*t; *b = 0; b = *t = *t+cnt; sz -= 2+cnt;
	if(sz < 2 || (cnt = r16(t)) > sz-2)
		goto error;
	stat->gid = (0 == cnt) ? NULL : (char*)*t; *b = 0; b = *t = *t+cnt; sz -= 2+cnt;
	if(sz < 2 || (cnt = r16(t)) > sz-2)
		goto error;
	if (0 == cnt) {
		stat->muid = NULL;
	} else {
		stat->muid = memmove(*t-1, *t, cnt); *b = stat->muid[cnt] = 0; 
	}
	*t = *t+cnt; sz -= 2+cnt;
	*t += sz;
	return 0;
